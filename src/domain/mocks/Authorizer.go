...
// GetAuthorizedServers provides a mock function with given fields: ctx, userID, authChecker
func (_m *Authorizer) GetAuthorizedServers(ctx context.Context, userID string, authChecker domain.AuthChecker) ([]int64, error) {
	ret := _m.Called(ctx, userID, authChecker)

	var r0 []int64
	if rf, ok := ret.Get(0).(func(context.Context, string, domain.AuthChecker) []int64); ok {
		r0 = rf(ctx, userID, authChecker)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]int64)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, string, domain.AuthChecker) error); ok {
		r1 = rf(ctx, userID, authChecker)
	} else {
		r1 = ret.Error(1)
	}
